-- Seteaza user-ul tau de admin pentru a asocia topicurile cu tine (va lua primul utilizator din baza de date)
SET @admin_id = (SELECT id FROM profiles LIMIT 1);

-- Inserare Categorii
INSERT IGNORE INTO forum_categories (id, title, slug, order_index, created_at)
VALUES 
(UUID(), 'Departamente', 'departamente', 1, CURRENT_TIMESTAMP),
(UUID(), 'Staff', 'staff', 2, CURRENT_TIMESTAMP);

-- Luam ID-urile exacte din baza de date pentru Categorii
SET @cat_dept_id = (SELECT id FROM forum_categories WHERE slug = 'departamente' LIMIT 1);
SET @cat_staff_id = (SELECT id FROM forum_categories WHERE slug = 'staff' LIMIT 1);

-- Inserare Forumuri
INSERT IGNORE INTO forums (id, category_id, title, slug, description, icon, threads_count, posts_count, order_index, created_at)
VALUES 
(UUID(), @cat_dept_id, 'Poliția Română', 'politia-romana', 'Aplicații, regulamente și anunțuri Poliția Română', '🚓', 2, 0, 1, CURRENT_TIMESTAMP),
(UUID(), @cat_dept_id, 'Spitalul General', 'spitalul-general', 'Aplicații, regulamente și anunțuri Spitalul General', '🚑', 2, 0, 2, CURRENT_TIMESTAMP),
(UUID(), @cat_staff_id, 'Echipa Staff', 'echipa-staff', 'Aplicații și informații despre echipa Staff', '🛡️', 2, 0, 1, CURRENT_TIMESTAMP);

-- Luam ID-urile exacte din baza de date pentru Forumuri (foarte important ca sa nu dea eroare de Foreign Key)
...
SET @forum_staff_id = (SELECT id FROM forums WHERE slug = 'echipa-staff' LIMIT 1);

-- Stergem versiunile mai vechi (in caz ca au ramas duplicate blocate de la insertul esuat anterior)
...
-- Inserare Model Aplicatie Medici
INSERT INTO forum_threads (id, forum_id, user_id, category, title, content, is_pinned, is_locked, views_count, replies_count, created_at, updated_at)
VALUES (
...
-- Inserare Model Aplicatie Staff
INSERT INTO forum_threads (id, forum_id, user_id, category, title, content, is_pinned, is_locked, views_count, replies_count, created_at, updated_at)
VALUES (
